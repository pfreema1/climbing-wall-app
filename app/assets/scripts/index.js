...
var loadProblemMenuContainerEl = document.getElementById("loadProblemMenuContainer");
...
var problemInfoProblemNameEl = document.getElementById("problemInfoProblemName");
...
function handleDrag(ev) {
    ...
    if (Date.now() - lastPinchTime > 300) {
        ...
        if (!isDragging) {
            ...
            var propValueStringLeft = window.getComputedStyle(elem).getPropertyValue("left");
            var propValueStringTop = window.getComputedStyle(elem).getPropertyValue("top");
            

            //chop off the px at the end of the string
            propValueStringLeft = propValueStringLeft.slice(0, propValueStringLeft.length - 2);
            propValueStringTop = propValueStringTop.slice(0, propValueStringTop.length - 2);
...
function changeColor(clickedElem) {
    ...
    var colorString = window.getComputedStyle(clickedElem, null).getPropertyValue("border");
    ...
    if (colorString.indexOf("none") != -1) {
        ...
        colorString = colorString.substring(colorString.lastIndexOf("rgb"), colorString.length);
...
function convertBorderStringToVw(str) {
    ...
    if (str.indexOf("none") != -1) {       //dont need to do anything since the hold does not have a border
        return str;
    } else {
...
function saveHoldObjectArray() {
    ...
    for (var i = 0; i < elements.length; i++) {
        ...
        var newBorderString = convertBorderStringToVw(window.getComputedStyle(elements[i], null).getPropertyValue("border"));
    ...
    if (problemNameInputEl.value != "" && authorNameEl.value != "" && gradeSelectEl.value != "") {
        //create form data
        ...
        request.open("POST", "http://paulfreeman.design/riverside/saveProblemToDB.php");
...
document.getElementById("saveHoldStatus").addEventListener("click", saveHoldObjectArray);
...
document.querySelector('.resetButtonContainer').addEventListener("click", function() {
...
function loadAllProblems() {
    ...
    request.open("GET", "http://paulfreeman.design/riverside/retrieveProblemsFromDB.php", true);

    request.send();

    request.onreadystatechange = function () {
        if (request.readyState == 4 && request.status == 200) {  //if request was successful and finished
...
function createLoadedProblemsArray() {
    ...
    for (var i = 0; i < dbResponseText.length; i++) {
        ...
        for (var j = 0; j < tempProblemArray.length; j++) {
            ...
            tempProblemArray[j] = tempProblemArray[j].substr(tempProblemArray[j].indexOf(":") + 1, tempProblemArray[j].length);
...
function populateProblemSelectDropdown() {
    ...
    for (var i = 0; i < loadedProblemsObjectArray.length; i++) {
        ...
        if(loadedProblemsObjectArray[i].grade != undefined) {
            selectElement.innerHTML += '<option value="' + loadedProblemsObjectArray[i].databaseID + '">' + loadedProblemsObjectArray[i].grade + ' // ' + loadedProblemsObjectArray[i].problemName + '</option>';
        }
    }
...
function loadSelectedProblem() {
    ...
    for (var key in tempHoldDataArray) {
        ...
        //get idString
        var idString = tempString.substring(tempString.indexOf("click"), tempString.indexOf(",") - 1);
        //get the border string
        var borderString = tempString.substring(tempString.indexOf("border"), tempString.length - 2);
...
document.getElementById("backButton_Create").addEventListener("click", function () {
    createMenuContainerEl.classList.add('animated');
    createMenuContainerEl.classList.add('fadeOutDown');
    mainMenuContainerEl.style.display = "flex";
    mainMenuContainerEl.classList.remove('fadeOutDown');
...
document.getElementById("backButton_Load").addEventListener("click", function () {
    loadProblemMenuContainerEl.classList.add('animated');
